...
- **In-progress states** (Daily Limit, Weekly Limit, Shift Active):
  - Resting card is **identical to `State=Active`** — running time, task name, blue-border card, pause button, and the full `Active` + `Break` chips row. The only addition is a small muted clock icon beside the timer digits (`Hover Indicator`), acting as the hover affordance.
  - The limit/shift counter is **hover-only**: hovering the clock icon reveals a tooltip with the counter text. The tooltip is not drawn on the variant; a Figma annotation on each variant documents the hover behavior and the exact counter copy. Revised 2026-06-10 — earlier bottom-caption and counter-chip-replacing-Break treatments were both rejected in review. Card height 146.
...
| `State=Daily Limit` | pause (active) | running `06:30:12` + clock affordance | "EverTech Drafting Proposals" | `Active` + `Break` (hover clock → tooltip "6h 30m / 8h today") |
...
| `State=Weekly Limit` | pause (active) | running + clock affordance | "EverTech Drafting Proposals" | `Active` + `Break` (hover clock → tooltip "32h 15m / 40h this week") |
...
| `State=Shift Active` | pause (active) | running + clock affordance | "EverTech Drafting Proposals" | `Active` + `Break` (hover clock → tooltip "Shift: 9:00 AM – 5:00 PM · 2h 30m left") |
