...
export type CounterProps = Omit<HTMLAttributes<HTMLSpanElement>, "color"> &
  CounterVariants & {
    /** Numeric count to render. Values above `max` show as `${max}+`. */
    count: number
    /** Cap before count is rendered as `${max}+`. Defaults to 99. */
    max?: number
    /** Accessible label. Defaults to `${display} new`. */
    "aria-label"?: string
  }
...
export const Counter = forwardRef<HTMLSpanElement, CounterProps>(function Counter(
  ...
  return (
    <span
      ...
      className={cn(counter({ size, kind, color }), padding, className)}
      {...rest}
    >
      {display}
    </span>
