...
const root = cva(
  ...
  ].join(" "),
  {
    variants: {
      ...
      surface: {
        default:
          "text-[var(--color-link-color)] hover:underline focus-visible:ring-[var(--color-primary-color)]",
        inverted:
          "text-[var(--color-text-color-on-inverted)] hover:underline focus-visible:ring-[var(--color-text-color-on-inverted)]",
        ...
        "on-tinted":
          "text-current underline focus-visible:ring-current",
      },
    },
...
export const Link = forwardRef<HTMLAnchorElement, LinkProps>(function Link(
  ...
  return (
    <a
      ...
      className={cn(root({ size, surface }), className)}
      {...rest}
    >
      {iconLeft && <Icon name={iconLeft} size="sm" className="text-current" aria-hidden />}
      {children != null && <span>{children}</span>}
      {iconRight && <Icon name={iconRight} size="sm" className="text-current" aria-hidden />}
    </a>
  )
})
