...
const DEFAULT_TAG_BY_VARIANT: Record<NonNullable<TextVariantProps["variant"]>, ElementType> = {
  h1: "h1",
  h2: "h2",
  h3: "h3",
  t1: "p",
...
type TextOwnProps<E extends ElementType> = TextVariantProps & {
  as?: E
  className?: string
}
...
function TextImpl<E extends ElementType = "p">(
  { as, variant, weight, link, className, ...rest }: TextProps<E>,
  ref: Ref<Element>,
) {
  const Tag = (as ?? DEFAULT_TAG_BY_VARIANT[variant ?? "t1"]) as ElementType
  return <Tag ref={ref} className={cn(text({ variant, weight, link }), className)} {...rest} />
}

export const Text = forwardRef(TextImpl) as <E extends ElementType = "p">(
