...
export type TextFieldSize = NonNullable<WrapperVariants["size"]>
export type TextFieldState = NonNullable<WrapperVariants["state"]>
...
export type TextFieldProps = NativeInputProps & {
  ...
  /** className for the outermost layout container (label + field + helper). */
...
const LEADING_ICON_SIZE = "sm" as const
...
export const TextField = forwardRef<HTMLInputElement, TextFieldProps>(
  function TextField(
    ...
    const helperTone: VariantProps<typeof helperCls>["tone"] =
      state === "error" ? "error" : state === "disabled" ? "disabled" : "default"
    ...
    return (
      <div className={cn("flex w-full flex-col gap-1", rootClassName)}>
        ...
        <div
          data-state={state}
          className={cn(wrapper({ size, state }), wrapperClassName)}
        >
          {leadingIcon && (
            <Icon
              name={leadingIcon}
