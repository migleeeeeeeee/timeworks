...
export type DividerProps = Omit<HTMLAttributes<HTMLDivElement>, "role"> &
  DividerVariants & {
    /**
     * When `true` (default) the divider is purely visual — `role="none"`,
     * not announced. Set `false` to expose it as a semantic separator
...
export const Divider = forwardRef<HTMLDivElement, DividerProps>(function Divider(
  { className, orientation: orientationProp, decorative = true, ...rest },
  ref,
) {
  const orientation = orientationProp ?? "horizontal"
  const semantic: HTMLAttributes<HTMLDivElement> = decorative
    ? { role: "none" }
