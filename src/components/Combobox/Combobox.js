 ...
 */
const surface = cva(["flex flex-col items-stretch gap-2 p-2 relative"], {
    variants: {
        size: {
...
function isBottomButtonConfig(value) {
...
export const Combobox = forwardRef(function Combobox({ options, value, defaultValue, onValueChange, searchValue, defaultSearchValue, onSearchChange, searchPlaceholder = "Placeholder text here", size = "md", dialog = true, button, emptyState, className, id, ...rest }, ref) {
    ...
    const [activeIndex, setActiveIndex] = useState(() => firstEnabled(filtered));
    ...
    useEffect(() => {
        ...
        const selectedIdx = filtered.findIndex((o) => o.value === selected && !o.disabled);
        if (selectedIdx >= 0) {
    ...
    const bottomButton = (() => {
        ...
        if (isBottomButtonConfig(button)) {
            return (_jsx(ListItem, { variant: "button", icon: button.icon, disabled: button.disabled, onClick: button.onClick, children: button.label }));
        }
        return button;
    })();
    const activeDescendantId = activeIndex >= 0 && activeIndex < filtered.length
        ...
        : undefined;
    return (_jsxs("div", { ref: ref, id: rootId, className: cn(surface({ size, dialog }), className), ...rest, children: [_jsx(Search, { size: SEARCH_SIZE[size ?? "md"], value: query, onValueChange: handleSearch, onKeyDown: handleKeyDown, placeholder: searchPlaceholder, "aria-label": "Filter options", role: "combobox", "aria-expanded": true, "aria-controls": listboxId, "aria-autocomplete": "list", "aria-activedescendant": activeDescendantId }), _jsx("div", { role: "listbox", id: listboxId, className: "flex flex-col items-stretch w-full", children: filtered.length === 0 ? (_jsx(ListItem, { variant: "information", children: emptyState ?? "No results" })) : (filtered.map((option, index) => {
                    const isSelected = option.value === selected;
                    const isHighlighted = index === activeIndex;
                    return (_jsx(ListItem, { id: optionId(index), role: "option", icon: option.icon, active: isSelected || isHighlighted, disabled: option.disabled, "aria-selected": isSelected, 
