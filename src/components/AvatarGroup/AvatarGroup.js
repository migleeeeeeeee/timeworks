 ...
 */
const counterOuter = cva("relative inline-flex shrink-0 rounded-full bg-[var(--color-primary-background-color)] ring-1 ring-[var(--color-primary-background-color)]", {
    variants: {
        size: {
            xs: "h-5",
            sm: "h-6",
...
const counterInner = cva("inline-flex size-full items-center justify-center rounded-full font-body whitespace-nowrap px-2", {
    variants: {
        size: {
        ...
        tone: {
            ...
            disabled: "bg-[var(--color-disabled-background-color)] text-[var(--color-disabled-text-color)]",
        },
    },
...
export const AvatarGroup = forwardRef(function AvatarGroup({ children, max, counterText, disabled = false, theme = "light", size = "md", className, ...rest }, ref) {
    ...
    const tone = disabled
        ...
        : theme === "dark"
            ? "dark"
            : "light";
    return (_jsxs("div", { ref: ref, "data-disabled": disabled || undefined, className: cn("inline-flex items-center", className), ...rest, children: [visible.map((child, i) => {
                const element = child;
                ...
                return (_jsx("span", { className: cn("inline-flex shrink-0", !isLast && overlap), children: cloned }, element.key ?? i));
            }), showCounter && (_jsx("span", { className: cn(counterOuter({ size })), children: _jsx("span", { className: cn(counterInner({ size, tone })), children: counterText ?? `+${remaining}` }) }))] }));
});
