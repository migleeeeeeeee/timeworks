...
type CellVariants = VariantProps<typeof cell>

export type TableCellSize = NonNullable<CellVariants["size"]>
export type TableCellState = NonNullable<CellVariants["state"]>
export type TableCellVariant = NonNullable<CellVariants["variant"]>

export type TableCellProps = HTMLAttributes<HTMLDivElement> &
  CellVariants & {
    /** Cell contents — text, chip, avatar, progress bar, etc. */
    children?: ReactNode
  }
...
export const TableCell = forwardRef<HTMLDivElement, TableCellProps>(
  function TableCell(
    ...
    const resolvedSize: TableCellSize = size ?? ctxSize ?? "md"
    return (
      <div
        ref={ref}
        role={role}
        ...
        className={cn(
          cell({ size: resolvedSize, variant, state, interactive, border }),
          className,
        )}
        {...rest}
      >
